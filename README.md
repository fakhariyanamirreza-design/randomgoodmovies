...
- `title_fa`: هدر بُلد یکدست + پرچم کشور سازنده، مثلاً `**تلقین** (2010) 🇺🇸` (ایموجی ژانر
  طبق درخواست کاربر حذف شد؛ نگاشت `content.genre_emoji` همچنان در کد هست و اگر دوباره به
  قالب اضافه کنی کار می‌کند).
...
> بلاک‌های `teaser_line` و `why_line` و هشتگ‌ها از قالب پیش‌فرض حذف شده‌اند (درخواست کاربر)؛
> موتورِ رندر و منطق آن‌ها همچنان در کد هست و اگر قالب را دوباره اضافه کنی کار می‌کند.

## پست تریلر (یک روز بعد)

برای هر پست معرفیِ دارای `message_id`، یک روز بعد (طبق `trailer.after_days`) یک **پست متنی جدا**
در مورد تریلر فیلم منتشر می‌شود:

- لینک تریلر از `/movie/{id}/videos` تی‌ام‌دی‌بی پیدا می‌شود (اولویت: تریلر رسمیِ یوتیوب، سپس
  هر تریلر یوتیوب بر اساس وضوح).
- پست به‌صورت **پیام متنی با لینک خام یوتیوب** ارسال می‌شود؛ تلگرام به‌صورت خودکار یک کارت
  ویدیوی **قابل پخش inline** داخل همان پیام می‌سازد (دانلود مستقیم و ارسال فایل ویدیو ممکن نیست).
- متن شامل: نام فیلم، لینک «مشاهده پست» (به‌صورت `<a href='https://t.me/<handle>/<message_id>'>`)،
  لینک خام یوتیوب، و footer.
- در هر اجرا فقط `trailer.max_per_run` پست تریلر ارسال می‌شود؛ اگر تریلری نبود، `trailer_skipped`
  ثبت و دوباره امتحان نمی‌شود. پست‌های قدیمی‌تر (بدون `message_id`) هرگز تریلر نمی‌گیرند.

## نقل‌قول روزانه

هر روز یک پست شامل **نقل‌قول سینمایی + عکس افقی (backdrop) از سکانس فیلم** منتشر می‌شود:
